...
def MinMaxScaler(df, training_set_length):
    ...
    df_copy.ix[:, 1:2+training_set_length] = df.ix[:, 1:2+training_set_length].apply(lambda x: (x - minValue) / (maxValue - minValue))
...
def NormalDistributionScaler(df, training_set_length):
    ...
    df_copy.ix[:, 1:2+training_set_length] = df.ix[:, 1:2+training_set_length].apply(lambda x: (x - mean) / std)
